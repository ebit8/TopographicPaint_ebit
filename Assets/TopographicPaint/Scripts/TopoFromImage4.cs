...
public class TopoFromImage4 : MonoBehaviour
{
        ...
        double gaussianKernelSize, double gaussianSigma)
    {
        ...
        if (contours.Count != 0) //輪郭線が1つでも抽出された場合
        {
            ...
            Imgproc.resize(mat_contour, mat_shrink, size_resize, 1, 1, Imgproc.INTER_CUBIC);

            //ガウスぼかし
            mat_blur = new Mat();
            Imgproc.GaussianBlur(mat_shrink, mat_blur,
                new Size(gaussianKernelSize, gaussianKernelSize), gaussianSigma);

            //拡大
            mat_dst = new Mat();
            size_resize = new Size(mat_src.cols(), mat_src.rows());
            Imgproc.resize(mat_blur, mat_dst, size_resize, 1, 1, Imgproc.INTER_LINEAR_EXACT);
            ...
            // for (int i = 0; i < contours.Count; i++)
            // {
            //     Imgproc.drawContours(mat_dst, contours, i, colors[i], 2);
            // }
            //
            // //bounding box
