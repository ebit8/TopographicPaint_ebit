...
public class TopoFromImage2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Ximgproc.thinning(mat_closing, mat_thin);
        ...
        for (idx = 1; idx < contours.Count; idx++)
        {
            ...
            if (hierarchy.get(0, idx)[3] == idx - 1)    //1つ前の輪郭線が親なら、高さ +1
            {
                currentLevel++;
            }
            ...
            if (hierarchy.get(0, idx)[1] == idx - 1 && hierarchy.get(0, idx)[3] != idx - 1)  //1つ前が前のやつでも親でもないなら、この輪郭線は閉じた円ではないので無効
            {
                contourLevel[idx - 1] = -1;
            }
