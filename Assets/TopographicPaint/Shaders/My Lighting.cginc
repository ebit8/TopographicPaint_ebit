...
InterpolatorsVertex MyVertexProgram (VertexData v) {
	...
	i.uv.zw = TRANSFORM_TEX(v.uv, _DetailTex);
		...
		displacement = displacement * _DisplacementStrength * _ContourMaxLevel;
...
#if defined(TESSELLATION_WITHOUT_NORMALMAP)
	float4 _ParallaxMap_TexelSize;
#endif

void InitializeFragmentNormal(inout Interpolators i) {
	#if defined(TESSELLATION_WITHOUT_NORMALMAP)
		//ここにグレースケール画像から法線を計算する処理を書く
		float height = _ParallaxStrength * 10;
		float2 du = float2(_ParallaxMap_TexelSize.x * 10, 0);
		float u1 = tex2D(_ParallaxMap, i.uv - du);
		float u2 = tex2D(_ParallaxMap, i.uv + du);
		float3 tu = float3(1, (u2 - u1) * height, 0);
		
		float2 dv = float2(0, _ParallaxMap_TexelSize.y * 10);
		float v1 = tex2D(_ParallaxMap, i.uv - dv);
		float v2 = tex2D(_ParallaxMap, i.uv + dv);
		float3 tv = float3(0, (v2 - v1) * height, 1);
...
FragmentOutput MyFragmentProgram (Interpolators i) {
	...
	float3 albedo = DiffuseAndSpecularFromMetallic(
		...
		// float3 topContour = smoothstep(0.0,contourWidth,frac((i.worldPos.y+0.9)* 1));  
		// float3 bottomContour = smoothstep(0.0, contourWidth,frac((i.worldPos.y+0.9)*  -1));  
		// float3 contourLines = topContour * bottomContour;
		// float3 contourColor = float3(0, 0, 0);
		// albedo *= contourLines;
		// albedo += (1-contourLines)*contourColor;
	#endif
	...
	// output.color = float4(i.normal, 1);
	#endif
